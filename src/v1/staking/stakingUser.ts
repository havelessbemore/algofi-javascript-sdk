  ...
  async loadState(userLocalStates: {}) {
    ...
    for (const [key, value] of Object.entries(userLocalStates)) {
      ...
      if (allStakingContracts.includes(appId)) {
        ...
        let stakingConfig = StakingConfigs[this.stakingClient.network].filter(
          stakingConfig => stakingConfig.appId === appId
        )[0]
        ...
        const staking = new Staking(
          this.algod,
          this.stakingClient,
          rewardsManagerAppId[this.stakingClient.network],
          stakingConfig
        )
