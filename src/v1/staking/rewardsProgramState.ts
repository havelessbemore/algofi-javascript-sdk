...
export default class RewardsProgramState{
	...
	constructor(stakingState: {}, rewardsProgramIndex: number) {
		...
		const nonFormattedRewardsCoefficient = stakingState[STAKING_STRINGS.rewards_coefficient_prefix + this.rewardsProgramIndex.toString()] 
		const bytesVer = new Uint8Array(Buffer.from(nonFormattedRewardsCoefficient, "base64"))
		const bigInt = bytesToBigInt(bytesVer)
		this.rewardsCoefficient = bigInt || 0n
	}
}
...
export class UserRewardsProgramState {
	...
	constructor(formattedUserLocalState: {}, rewardsProgramIndex: number, staking: Staking, userScaledTotalStaked: number) {
		...
		const nonFormattedRewardsCoefficient = formattedUserLocalState[STAKING_STRINGS.user_rewards_coefficient_prefix + this.rewardsProgramIndex.toString()] 
		const bytesVer = new Uint8Array(Buffer.from(nonFormattedRewardsCoefficient, "base64"))
		const bigInt = bytesToBigInt(bytesVer)
		this.userRewardsCoefficient = bigInt || 0n
		...
		this.userUnrealizedRewards = Number(((globalCoefficient - userCoefficient) * BigInt(userScaledTotalStaked)) + BigInt(this.userUnclaimedRewards))
